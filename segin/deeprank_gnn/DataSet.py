...
class HDF5DataSet(Dataset):
    ...
    def load_one_graph(self, fname, mol):
        ...
        try:
            ...
            print("edge features not found in the file", self.database[0])
            f5.close()
            return None
        ...
        if "clustering" in grp.keys():
            if self.clustering_method in grp["clustering"].keys():
                if (
                    "depth_0"
                    in grp["clustering/{}".format(self.clustering_method)].keys()
                    and "depth_1"
                    in grp["clustering/{}".format(self.clustering_method)].keys()
                ):
    ...
    def filter(self, molgrp):
        ...
        for cond_name, cond_vals in self.dict_filter.items():
            ...
            try:
                ...
                print("   :Filter %s not found for mol %s" % (cond_name, molgrp))
